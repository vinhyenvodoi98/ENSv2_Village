...
export function AddressWorldPanel({ address }: { address: `0x${string}` }) {
  ...
  useEffect(() => {
    ...
    // A castle focused/zoomed-in on `/` or `/ens/[name]` is store state, not page state — drop it
    // so this page's map opens on the default centered framing instead of inheriting whatever
