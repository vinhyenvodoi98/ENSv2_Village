...
export function RolesSummary({ state }: { state: EnsNameState }) {
  ...
  return (
    ...
    >
      {!isConnected || !address ? (
        ...
        <p className="text-sm text-white/50">
          No registry governs this name yet, so there is no resource to hold roles on.
        </p>
      ) : state.resource === null ? (
        <p className="text-sm text-white/50">
...
function RoleGroup({
  ...
  return (
    <div>
      ...
      <p className="mt-2 font-mono text-[11px] break-all text-white/25">
        raw bitmap 0x{bitmap.toString(16)}
      </p>
    </div>
  );
}
